...
namespace UniversityCourseAndResultManagementApp.Gateway
{
    public class ClassRoomGateway : Gateway
    {
        ...
        {
            Query =
                "SELECT r.RoomNo,a.Day,a.FromTime,a.ToTime FROM AllocateClassroom a JOIN Room r ON r.Id = a.RoomId WHERE a.CourseId = @courseID AND Allocate = @allocate";
            Command = new SqlCommand(Query, Connection);
