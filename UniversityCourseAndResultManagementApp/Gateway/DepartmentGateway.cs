...
namespace UniversityCourseAndResultManagementApp.Gateway
{
    public class DepartmentGateway : Gateway
    {
        ...
        {
            Query = "SELECT Id,CreditToBeTaken FROM Teacher";
            Command = new SqlCommand(Query, Connection);
            Connection.Open();
            Reader = Command.ExecuteReader();
            List<Teacher> teachers = new List<Teacher>();
            ...
            while (Reader.Read())
            {
                ...
                aTeacher.CreditTake = Convert.ToDouble(Reader["CreditToBeTaken"]);
        ...
        public int UpdateRemainingCredit(Teacher aTeacher)
        {
            Query = "Update Teacher SET RemainingCredit = @remainigCredit WHERE Id = @id";
            Command = new SqlCommand(Query, Connection);

            Command.Parameters.Clear();
            Command.Parameters.Add("remainigCredit", SqlDbType.Decimal);
            Command.Parameters["remainigCredit"].Value = aTeacher.CreditTake;
            Command.Parameters.Add("id", SqlDbType.Int);
            Command.Parameters["id"].Value = aTeacher.Id;
            Connection.Open();
            int rowAffected = Command.ExecuteNonQuery();
            Connection.Close();
            return rowAffected;
