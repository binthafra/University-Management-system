...
namespace UniversityCourseAndResultManagementApp.Gateway
{
    public class AllocateClassroomGateway:Gateway
    {
        ...
        //public bool IsRoomAllocated(int allocateClassroomId)
        //{
        //    Query = "SELECT * FROM AllocateClassroom WHERE RoomId = @roomId AND Allocate =@allocate ";
        //    Command = new SqlCommand(Query, Connection);
        //    Query = "SELECT * FROM AllocateClassroom WHERE RoomId = @roomId";
        //    Command = new SqlCommand(Query, Connection);

        //    Command.Parameters.Clear();
        //    Command.Parameters.Add("roomId", SqlDbType.Int);
        //    Command.Parameters["roomId"].Value = allocateClassroomId;
        //    Command.Parameters.Add("allocate", SqlDbType.Bit);
        //    Command.Parameters["allocate"].Value = 1;
        //    Connection.Open();
        //    Reader = Command.ExecuteReader();
